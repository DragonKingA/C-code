...
/*全局变量声明区*/
static HWND hBtn1,hBtn2,hBtn3,hBtn4,hBtn5,hBtn6,hBtn7,hBtn8,hBtn9;//定义窗口句柄类，作为按钮（按钮是一种子窗口）
int board[3][3];//构建棋盘数组
int ButtonLock[3][3];//存储按钮开闭状态
int ret = 0;//存储双方对局结果，O胜\X胜\平局


//以下声明用于实现游戏具体功能的函数==================================================================================================================
//1.初始化棋盘数组(0则为O,1则为X,-1则为空)
void BoardInit(void){//棋盘初始化为空,-1
    memset(board,-1,sizeof(board));
}

//2.落子交替 及 按钮锁死系统 ,-1则锁死，0则开放,初始化为0
void BtLockInit(void){//初始化按钮状态为开放
    memset(ButtonLock,0,sizeof(ButtonLock));
}

int X_or_O(HWND button,int time,int n1,int n2){//n1为按钮横向标号,n2为按钮竖向标号
    if(ButtonLock[n1][n2]==0){
        if(time % 2 == 0){
    ...
    LPARAM lParam)
{
    ...
    switch(message){
        case WM_CREATE:
            ...
            //将逻辑字体应用到按钮
            SendMessage(hBtn1,WM_SETFONT,(WPARAM)hFont,lParam);
        ...
        case WM_COMMAND://按钮事件
            ...
            switch(wmld){
                //各个按钮点击事件！！！
                case 2:
                    if(X_or_O(hBtn1,times,0,0)) times++;           
        ...
        case WM_PAINT://若发送了WM_PAINT消息(message = WM_PAINT)时，则绘制窗口
        ...
        case WM_DESTROY://若发送了WM_DESTROY消息,则销毁窗口
            DeleteObject(hFont);
            PostQuitMessage(0);
...
//可以运用fopen()等函数将对局数据写入本地txt中，然后用户可以进行读取
//https://blog.csdn.net/shulianghan/article/details/121388293?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522166394303016782390537918%2522%252C%2522scm%2522%253A%252220140713.130102334.pc%255Fall.%2522%257D&request_id=166394303016782390537918&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~first_rank_ecpm_v1~rank_v31_ecpm-1-121388293-null-null.142^v50^control,201^v3^add_ask&utm_term=%E5%A6%82%E4%BD%95%E6%89%8B%E5%8A%A8%E5%88%B7%E6%96%B0%E7%BC%93%E5%86%B2%E5%8C%BA&spm=1018.2226.3001.4187
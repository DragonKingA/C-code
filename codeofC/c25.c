...
void main(){
    ...
    int * const pp3 = &w1[2];//* const表示指针pp3指向的地址不会改变,即pp3自身不变,但不影响改变*pp3和w1[2]的值
    const int *pp4 = &w1[4];//const *表示能改变pp4指向的地址 但不能通过指针pp4来改变*pp4的值，只能直接改变w1[4]的值。
    //上句等效于int const *pp4 = &w1[4],看谁被const修饰只需看 * 和 const两者的相对位置,int的位置不影响
    pp4 = &w1[5];

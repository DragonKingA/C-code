...
int cmp_int(const void *e1 , const void *e2){//因为e1,e2是无类型指针，系统不知道该指针占几个字节，故要先进行强制类型转换才能使用
    ...
    //而return *(int *)e1 > *(int *)e2 ;不包含两者相等即返回0的情况，故会出现相等者仍然发生交换的情况
}
int cmp_struct_age(const void* e1, const void* e2){
    if(((struct stu *)e1)->age != ((struct stu *)e2)->age)
        ...
        //防止相等的两者仍然发生交换
    //return ((struct stu *)e1)->age > ((struct stu *)e2)->age ;无法囊括所有范围
}
...
cmp函数的返回值，<0(不进行置换)，0(不进行置换)，>0(进行置换)。

对不同类型排序后return时的写法:
int     :    return *(int *)a - *(int *)b
double  :    return  ( *(double *)a - *(double *)b ) > 0 ? 1 : -1 ; //数值排序同值交换不影响最终结果
char    :    return (*(char *)a - *(char *)b);
struct s:    if( ((struct s *)a) -> data != ((struct s *)b) -> data )
                return ((struct s *)a) -> data  -  ((struct s *)b) -> data  >  0  ?  1 : -1 ; (这里data为数值类型)
             else
                return ((struct stu *)a)->sortindex - ((struct stu *)b)->sortindex ; 
                //这里sortindex在排序前初始化，代表着元素顺序，这样可以保证同值下不发生交换即保持原顺序
*/
